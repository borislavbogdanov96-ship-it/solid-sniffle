...
**Codebase map:** If a `NAVIGATION.md` exists at the project root (generated by `/agent-navigation-map`), read it before exploring the directory tree — it's a one-file index (spatial map, structured index, relationships, domain knowledge) meant to save navigation tokens. Cross-check against the real filesystem per its embedded protocol rather than trusting it blindly if it looks stale.
