...
**4d — Navigation map refresh (conditional, never creates):**
Check: does `NAVIGATION.md` exist at this project's root (previously generated by
`/agent-navigation-map`)?
- If no → skip entirely. Wrap-up never creates a `NAVIGATION.md` from scratch — generating one
  for the first time stays manual/on-demand via `/agent-navigation-map`.
- If yes, but nothing outside `.claude/` changed this session → note "Navigation map: up to
  date" and skip.
- If yes and this session added/moved/removed files or changed structure → refresh it: diff the
  existing file against the current filesystem, update only the sections that actually changed
  (don't reauthor untouched sections), bump `last_generated`. If Step 2 logged friction about
  navigating this project, prioritize fixing whatever section caused it.
- Report one line: "Navigation map: refreshed" / "Navigation map: up to date" / "Navigation map:
  n/a (none exists)".
